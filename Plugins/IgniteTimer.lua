...
L:RegisterTranslations("enUS", function() return {
	...
	["Gives timer for Ignite Aura."] = true,
	["Enable"] = true,
	["Enable timers."] = true,
	["Target only"] = true,
	...
	["Anchors the ignite timer to your scorch timer."] = true,
	["Reverse"] = true,
...
-- 	["Gives timer for Ignite Aura."] = "Temporizador para vulnerabilidad al fuego de Chamuscar",
-- 	["Enable"] = "Activar",
...
-- 	["Ignite Timer"] = "Temporizador de Chamuscar",

-- 	firevuln_test = "(.+) está afligido por Vulnerabilidad de Fuego.",
-- 	ignite_test	= "^Tu Chamuscar (%a%a?)\su (.+) para",
...
BigWigsIgniteTimer = BigWigs:NewModule(name)
BigWigsIgniteTimer.defaultDB = {
	growup = false,
	scale = 1.0,
	texture = "BantoBar",
	currentonly = true,
...
BigWigsIgniteTimer.consoleCmd = L["IgniteTimer"]
BigWigsIgniteTimer.consoleOptions = {
	type = "group",
	name = L["Ignite Timer"],
	desc = L["Gives timer for Ignite Aura."],
	args   = {
		enable = {
			type = "toggle",
			name = L["Enable"],
			...
			order = 1,
			get = function() return BigWigsIgniteTimer.db.profile.enable end,
			set = function(v) BigWigsIgniteTimer.db.profile.enable = v end,
		},
		currentonly = {
			type = "toggle",
			name = L["Target only"],
			...
			order = 2,
			get = function() return BigWigsIgniteTimer.db.profile.currentonly end,
			set = function(v) BigWigsIgniteTimer.db.profile.currentonly = v end,
		},
		spacer = {
			type = "header",
			name = " ",
		...
		anchor = {
			...
			order = 5,
			func = function() BigWigsIgniteTimer:ShowAnchors() end,
		},
		reset = {
			type = "execute",
			...
			order = 6,
			func = function() BigWigsIgniteTimer:ResetAnchor() end,
		},
		anchorscorch = {
			type = "toggle",
			name = L["Anchor with ScorchTimer"],
			desc = L["Anchors the ignite timer to your scorch timer."],
			order = 7,
			get = function() return BigWigsIgniteTimer.db.profile.anchorscorch end,
			set = function(v) BigWigsIgniteTimer.db.profile.anchorscorch = v end,
		},
		spacer2 = {
			type = "header",
			name = " ",
		...
		growup = {
			...
			order = 13,
			get = function() return BigWigsIgniteTimer.db.profile.growup end,
			set = function(v)
				BigWigsIgniteTimer.db.profile.growup = v
				candybar:SetCandyBarGroupPoint("IgniteTimerGroup", v and "BOTTOM" or "TOP", BigWigsIgniteTimer.frames.anchor, v and "TOP" or "BOTTOM", 0, 0)
				candybar:SetCandyBarGroupGrowth("IgniteTimerGroup", v)
			end,
		...
		reverse = {
			...
			order = 14,
			get = function() return BigWigsIgniteTimer.db.profile.reverse end,
			set = function(v) BigWigsIgniteTimer.db.profile.reverse = v end,
		--passValue = "reverse",
		},
		scale = {
			type = "range",
			...
			max = 2.0,
			step = 0.1,
			get = function() return BigWigsIgniteTimer.db.profile.scale end,
			set = function(v) BigWigsIgniteTimer.db.profile.scale = v end,
		},
		texture = {
			type = "text",
			name = L["Texture"],
			...
			order = 16,
			get = function() return BigWigsIgniteTimer.db.profile.texture end,
			set = function(v) BigWigsIgniteTimer.db.profile.texture = v end,
			validate = surface:List(),
		},
	},
}

BigWigsIgniteTimer.revision = 20001
BigWigsIgniteTimer.external = true

BigWigsIgniteTimer.target = nil
BigWigsIgniteTimer.playerName = nil
BigWigsIgniteTimer.igniteTalent = nil
BigWigsIgniteTimer.lastVictim = nil
BigWigsIgniteTimer.debuffs = {}
-----------------------------------------------------------------------
--      Initialization
-----------------------------------------------------------------------

function BigWigsIgniteTimer:OnRegister()
...
function BigWigsIgniteTimer:OnEnable()
	...
	if isMage then
		...
		self.igniteTalent = self:CheckTalents()
	end
	...
	self:ThrottleSync(0.2, syncName.ignite)

end

function BigWigsIgniteTimer:OnDisable()
	self:HideAnchors()
end
...
--      Event Handlers
-----------------------------------------------------------------------
function BigWigsIgniteTimer:PlayerDamageEvents(msg)
	--[[
	...
	local start, ending, spell, victim = string.find(msg, L["ignite_test"])
	if spell and victim and self:IsIgniteSpell(spell) and self.target == victim then
		--self:DebugMessage("Scorch - ".. victim)
		self.lastVictim = victim
		self:ScheduleEvent("IgniteTimerDelayedBar", self.DelayedBar, 0.2, self)
	end
...
function BigWigsIgniteTimer:DelayedBar()
	if not self.igniteTalent then
		self.igniteTalent = self:CheckTalents()
	end
	if self.igniteTalent then
		self:Sync(syncName.ignite.." "..self.lastVictim)
	end
end

function BigWigsIgniteTimer:PLAYER_REGEN_ENABLED()
	self.target = nil
	self:StopAllBars()
	self.debuffs = {}
end

function BigWigsIgniteTimer:CHARACTER_POINTS_CHANGED()
	self.igniteTalent = self:CheckTalents()
end

function BigWigsIgniteTimer:RecheckTargetChange()
	...
	if target ~= self.target then
		...
		if victim and timeleft and self.db.profile.enable then
			self:StartBar(victim, timeleft, "Interface\\Icons\\Spell_Fire_Incinerate")
		end
	end
end
-- reset data if you change your target
function BigWigsIgniteTimer:PLAYER_TARGET_CHANGED(msg)
	if not self:IsEventScheduled("IgniteTimerReckeckTargetChange") then
		self:ScheduleEvent("IgniteTimerReckeckTargetChange", self.RecheckTargetChange, 0.1, self)
	end
end

function BigWigsIgniteTimer:BigWigs_RecvSync(sync, rest, nick)
	if sync == syncName.ignite then
		self.debuffs[rest] = GetTime()
		if rest == self.target and self.db.profile.enable then
			self:StartBar(rest, timer.ignite-0.2, "Interface\\Icons\\Spell_Fire_Incinerate")
		end
	end
end
...
--      Util
-----------------------------------------------------------------------
function BigWigsIgniteTimer:GetTargetInfo()
	for k, v in pairs(self.debuffs) do
		if k == self.target then
			local timeleft = timer.ignite-(GetTime() - v)-0.2
			if timeleft > 0 then
				return k, timeleft
			end
		end
...
function BigWigsIgniteTimer:CheckTalents()
	nameTalent, icon, tier, column, currRank, maxRank= GetTalentInfo(2,3);
...
function BigWigsIgniteTimer:Test()
	self:StartBar("Mob 1", timer.ignite, "Interface\\Icons\\Spell_Fire_Incinerate")
	self:StartBar("Mob 2", timer.ignite/2, "Interface\\Icons\\Spell_Fire_Incinerate")
end

function BigWigsIgniteTimer:IsIgniteSpell(spell)
...
function BigWigsIgniteTimer:SetScale(msg, supressreport)
...
function BigWigsIgniteTimer:ToggleUp(supressreport)
...
function BigWigsIgniteTimer:SetupFrames()
	...
	local frame = CreateFrame("Frame", "IgniteTimerBarAnchor", UIParent)
	...
	self.frames.anchor.candyBarGroupId = "IgniteTimerGroup"
...
function BigWigsIgniteTimer:ResetAnchor(specific)
...
function BigWigsIgniteTimer:SavePosition()
	if not self.frames.anchor then self:SetupFrames() end
...
function BigWigsIgniteTimer:RestorePosition()
	local x = self.db.profile.posx
	local y = self.db.profile.posy
...
function BigWigsIgniteTimer:Disable()
	self:StopAllBars()
end

function BigWigsIgniteTimer:ShowAnchors()
	if not self.frames.anchor then self:SetupFrames() end
	self.frames.anchor:Show()
end

function BigWigsIgniteTimer:HideAnchors()
...
function BigWigsIgniteTimer:StartBar(text, time, icon)
	if not text or not time then return end
	local id = "IgniteTimerBar "..text
	if not self.frames.anchor then self:SetupFrames() end


	if self.db.profile.anchorscorch then 
		groupId = "ScorchTimerGroup"
	else
		groupId = self.frames.anchor.candyBarGroupId
	end
...
function BigWigsIgniteTimer:StopBar(text)
	if not text and type(text) ~= "string" then return end
	candybar:UnregisterCandyBar("IgniteTimerBar "..text)
end

function BigWigsIgniteTimer:StopAllBars()
	for i=1, table.getn(barCache) do
		BigWigsIgniteTimer:StopBar(barCache[i])
	end
	barCache = {}
end
